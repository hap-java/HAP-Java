...
public class ValveService extends AbstractServiceImpl {
    ...
    public ValveService(Valve valve, String serviceName) {
        ...
        addCharacteristic(new ActiveCharacteristic(valve::getValveActive, a -> valve.setValveActive(a),
                valve::subscribeValveActive, valve::unsubscribeValveActive));
        addCharacteristic(new ValveTypeCharacteristic(valve));
        addCharacteristic(new InUseCharacteristic(valve::getValveInUse, valve::subscribeValveInUse,
                valve::unsubscribeValveInUse));
        ...
        if (valve instanceof ValveWithTimer) {
            ...
            addCharacteristic(new RemainingDurationCharacteristic(vwt::getRemainingDuration,
                    vwt::subscribeRemainingDuration, vwt::unsubscribeRemainingDuration));
        }
    }
}
