...
public class FanService extends AbstractServiceImpl {
  ...
  public FanService(FanAccessory fan) {
    this(
        new ActiveCharacteristic(
            () -> fan.isActive().thenApply(s -> s ? ActiveEnum.ACTIVE : ActiveEnum.INACTIVE),
            (v) -> fan.setActive(v == ActiveEnum.ACTIVE),
            fan::subscribeActive,
            fan::unsubscribeActive));
  }
